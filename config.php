...
function getMainKeys(){
    ...
    $mainKeys = array_merge($mainKeys,[
        ...
        [['text'=>$buttonValues['sharj'],'callback_data'=>"increaseMyWallet"], ['text'=>$buttonValues['invite_friends'],'callback_data'=>"inviteFriends"]],
        [['text'=>$buttonValues['application_links'],'callback_data'=>"reciveApplications"],['text'=>$buttonValues['my_info'],'callback_data'=>"myInfo"]],
    ...
    foreach($serverIp as $server_ip){
        ...
        if($inbound_id == 0) {
            ...
            if($protocol == 'trojan'){
                ...
                if($netType == 'tcp' and $header_type == 'http') $psting .= '&headerType=http';
                if(strlen($sni) > 1) $psting .= "&fp=chrome&alpn=http%2F1.1&sni=$sni";
                $outputlink = "$protocol://$uniqid@$server_ip:$port?type=$netType&path=%2Ffey&host=$sni&security=$tlsStatus{$psting}#$remark";
